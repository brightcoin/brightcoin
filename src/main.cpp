...
uint256 hashGenesisBlock("0xb472a407d40639a2c311f0efaef43e250be236360fc7236b494f51c64b18959c");
uint256 hashTestnetGenesisBlock("0xc71d956000767a5dd9c9581edb0a876fc4649f3f4da11f33aafc568371e4d148");
...
{
    int64 nSubsidy = COINS_PER_BLOCK * COIN;
...
#if defined(PREMINE_COINS)
    if (nHeight == 1)
	nSubsidy = PREMINE_COINS * COIN;
#endif

    return nSubsidy + nFees;
}

static void DoubleToNumeratorDenominator(double inDouble, long long *outNumerator, long long *outDenominator)
{
    double fPart;
    int expo; // exponent
    long long lExpo;
...
static const int64 nTargetSpacing = TARGET_SPACING; // 2.5 minutes
...
{
    // Brightcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ecns.cn 03/Mar/2014 CPC newspaper slams Abe's politics";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1393899612;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084937036;

        if (fTestNet)
        {
            block.nTime    = 1393890612;
            block.nNonce   = 385517995;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x9f5507f311158b79d7832d2f82c8dca9427031ba7880574978cbec7984552f36"));

#if defined(GENERATE_GENESIS_BLOCK)
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xdc }; // Brightcoin: increase each by adding 3 to bitcoin's value.
